...
项目的核心目标不是把大量源码片段直接塞进大模型上下文，而是先构建可查询的语义图谱，再通过摘要向量召回定位相关节点，沿图谱扩展局部关系，并把受控工具返回的图谱证据交给模型回答。这样可以减少无关上下文，提高回答依据的可解释性。
...
5. 检索问答：LangGraph chat 工具链先使用 `Summary.embedding` 向量索引召回种子节点，再通过受控的一跳邻接和单关系查询累积 evidence，最后基于 evidence 生成中文回答。
